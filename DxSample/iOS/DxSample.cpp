...
char StringBuf[ STRBUF_HEIGHT ][ STRBUF_WIDTH * 3 + 1 ] ;	// 仮想テキストバッファ
...
int ios_main( void )
{
	...
	while( ProcessMessage() == 0 && CheckHitKey( KEY_INPUT_ESCAPE ) == 0 )
	{
		...
		if( EndFlag == 0 )
		{
			...
			// ボタン押し待ちフラグがたっていた場合はタップされるまでここで終了
			if( KeyWaitFlag == 1 )
			{
				if( ProcessMessage() == 0 && GetTouchInputNum() != 0 ) 
				{
					// ボタンが押されていたら解除
					KeyWaitFlag = 0 ;
				}
			...
			else
			{
				...
				default :	// その他の文字
					...
					OneMojiBuf[ 2 ] = String[ SP ][ CP + 2 ] ;
					OneMojiBuf[ 3 ] = '\0' ;

					// １文字テキストバッファに代入
					StringBuf[ CursorY ][ CursorX * 3     ] = OneMojiBuf[ 0 ] ;
					StringBuf[ CursorY ][ CursorX * 3 + 1 ] = OneMojiBuf[ 1 ] ;
					StringBuf[ CursorY ][ CursorX * 3 + 2 ] = OneMojiBuf[ 2 ] ;
